        ...
        if phase == "SCANNING":
            ...
            if 1040 <= x <= 1150 and 540 <= y <= 590:
                if current_face_idx > 0:
                    ...
                    cube_state[target_idx] = ["unknown"] * 9
                    ...
                    for i in range(current_face_idx):
                        face_name = faces[i]

                        target_idx, vis_idx = face_indices[face_name]

                        formatted_face = [
                            color[0].upper() if color != "unknown" else "unknown"
                            for color in cube_state[target_idx]
                        ]

                        draw_face(ax, formatted_face, vis_idx)

                return

            # --- "EDIT DONE" Button ---
            if 870 <= x <= 1020 and 540 <= y <= 590:
                phase = "SOLVING"
...
while True:
    ...
    if phase == "SCANNING":
            ...
            # "SCAN FACE" 
            cv.rectangle(blank, (700, 540), (850, 590), NAVY_COLOR, thickness=cv.FILLED)
            cv.rectangle(blank, (700, 540), (850, 590), (255, 255, 255), thickness=2)
            cv.putText(blank, "SCAN FACE", (715, 572), cv.FONT_HERSHEY_COMPLEX, 0.6, (255, 255, 255), 2)

            # "EDIT DONE" 
            cv.rectangle(blank, (870, 540), (1020, 590), (0, 180, 0), thickness=cv.FILLED)
            cv.rectangle(blank, (870, 540), (1020, 590), (255, 255, 255), thickness=2)
            cv.putText(blank, "EDIT DONE", (890, 572), cv.FONT_HERSHEY_COMPLEX, 0.6, (255, 255, 255), 2)
            
            #UNDO 
            cv.rectangle(blank, (1040, 540), (1150, 590), (100,0,200), thickness=cv.FILLED)
            cv.rectangle(blank, (1040, 540), (1150, 590), (255, 255, 255), thickness=2)
            cv.putText(blank, "UNDO", (1065, 572), cv.FONT_HERSHEY_COMPLEX, 0.6, (255, 255, 255), 2)
        ...
        if time.time() - start_time >= 3:
            ...
            cv.putText(blank, f"Move {sol_idx + 1} / {len(solution)}: {current_move}", (800, 250), cv.FONT_HERSHEY_COMPLEX, 0.7, (255, 255, 255), 2)

    # "PREV MOVE" 
            cv.rectangle(blank, (750, 400), (910, 450), NAVY_COLOR, thickness=cv.FILLED)
            cv.rectangle(blank, (750, 400), (910, 450), (255, 255, 255), thickness=2)
            cv.putText(blank, "PREV MOVE", (770, 432), cv.FONT_HERSHEY_COMPLEX, 0.6, (255, 255, 255), 2)

            # "NEXT MOVE" Button
            cv.rectangle(blank, (950, 400), (1100, 450), (0, 180, 0), thickness=cv.FILLED)
            cv.rectangle(blank, (950, 400), (1100, 450), (255, 255, 255), thickness=2)
            cv.putText(blank, "NEXT MOVE", (970, 432), cv.FONT_HERSHEY_COMPLEX, 0.6, (255, 255, 255), 2)
